    ...
    try {
      ...
      if(!user) return toast("Login to send message")
        ...
        const {data} = await axios.post(`/api/message/${mode}`,{chatId:selectedChat._id,
          ...
          if(data.success){
            setMessages(data.chat.messages);        // 👈 use all messages
    ...
    max-md:mt-14 2xl:pr-40'>
        ...
        gap-4 items-center'>
            <select onChange={(e)=>setMode(e.target.value)} value={mode} className='text-sm pl-3 pr-2 outline-none'>
              <option  className='bg-grey-400  dark:bg-purple-900' value="Image">Image</option>
              <option className='bg-grey-400  dark:bg-purple-900' value="Text">Text</option>
            </select>
            <input onChange={(e)=>setPrompt(e.target.value)} type="text" placeholder='Type your Prompt here...'
            className='flex-1 w-full text-sm outline-none' value={prompt} required/>
