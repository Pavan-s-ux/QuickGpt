...
const Sidebar = ({isMenuOpen , setIsMenuOpen}) => {
    const {chats, setSelectedChat, user, theme, setTheme,navigate,createNewChat
        ,axios,setChats,fetchUsersChats,setToken,token
    } = useAppContext();
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5  dark:bg-gradient-to-b dark:from-[#242124]/30 dark:to-[#000000]/30 border-r border-[#80609f]/30
    backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'} `}>
        ...
        <button onClick={createNewChat} className='flex justify-center items-center w-full py-1 mt-10 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F6]
        text-sm rounded-md cursor-pointer'>
            <span className='m-1 text-l'>+</span>
            New Chat</button>
        ...
        <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-2'>
            {
                ...
                :chat.name.toLowerCase().includes(search.toLowerCase())).map((chat)=>(
                    <div onClick={()=> {navigate('/');  setSelectedChat(chat); setIsMenuOpen(false)}}   key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 
                    rounded-md cursor-pointer flex justify-between group '>
                        ...
                        <img onClick={e=> toast.promise(deleteChat(chat._id, e),{loading: 'deleting...'})} src={assets.bin_icon} className='hidden group-hover:block w-4  cursor-pointer not-dark:invert' alt="" />
                    </div>
                ))
            }
        </div>
