  ...
  return (
    <div className='p-6 pt-12 xl:px-12 2xl:px-20 w-full mx-auto h-full overflow-y-scroll'>
      ...
      {images.length > 0 ?(
        <div className='flex flex-wrap max-sm:justify-center gap-5'>
          {images.map((image,index)=>(
            <a href={image.imageUrl} key={index} target='_blank' className='relative group block rounded-lg overflow-hidden
            border border-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md
            transition-shadow duration-300'>
              <img src={image.imageUrl} alt="" className='w-full h-40 md:h-50
      ...
      (
       ...
       dark:text-purple-200 mt-10'>No Images Available</p> 
      )}
